...
void BOARD_BootClockRUN(void)
{
    ...
    /* FIXED 2026-07-22 -- this file was generated by a newer MCUXpresso
     * Config Tools version (Clocks v13.0) than the confirmed-working
     * reference project (Clocks v10.0), which unconditionally forces
     * the 600MHz clock path regardless of what this specific chip's
     * own fuses say it's actually rated for. The reference reads
     * OCOTP->CFG3 (the chip's own speed-grade fuse) and only selects
     * 600MHz if the silicon is actually fused for it, falling back to
     * 528MHz (industrial-temp-rated parts) otherwise. Running the
     * unconditional 600MHz path on chip that's only fused for 528MHz
     * runs the entire clock tree (PLLs, IPG_CLK, everything derived
     * from it -- including MDIO timing and ENET) out of spec, which is
     * consistent with marginal/unreliable PHY and networking behavior
     * that persisted across chip erases and reflashes, since it was
     * baked into generated source, not flash content. Restored to
     * match the reference's fuse-checking logic exactly. */
    ...
    /* FIXED 2026-07-22 -- same reasoning as the PrePeriphMux fix above.
     * The newer Config Tools version hardcodes SystemCoreClock to the
     * TARGET value, assuming it was actually achieved, rather than
     * reading back what the hardware genuinely ended up running at.
     * The reference calls SystemCoreClockUpdate() (real hardware
     * readback) instead -- restored to match. */
