...
static void process_time_sync(app_context_t *app)
{
...

    {
        ...
        if (ds3231_rt1062_poll_rollover(&edge_ms)) {
            ...
            {
                /* CORRECTED 2026-07-22 (re-applied 2026-07-23), per
                 ...
                 * app_update_battery_percent()'s own fix below:
                 ...
                 * now. With APP_ENABLE_NRF_SPI off,
...
{
    /* CORRECTED 2026-07-22 (re-applied 2026-07-23), per explicit
     * instruction -- this used to defer whenever the nRF READY line was
     * asserted ("don't let slow work delay draining a pending nRF
     * read", ported faithfully from the original's own
     * `if(!BitRdPortI(PBDR,3))` wrapping this exact block). Confirmed
     * wrong: NRF_READY has nothing to do with the MAX17303 fuel-gauge
     * I2C read -- it exists solely to tell this port when to poll the
     * nRF chip for new chip reads, unrelated buses entirely. That gate
     * was also the actual root cause of "V=0 forever" over TCP: this
     * board's NRF_READY reads permanently asserted, which silently
...
static void process_periodic_checks(app_context_t *app, uint32_t now_ms)
{
     ...
     * CORRECTED 2026-07-22 (re-applied 2026-07-23), per explicit
     * instruction -- the original's own `!BitRdPortI(PBDR,3)` gate
     * around GetGPS_Signal() (restored 2026-07-20) is now removed, same
     * reasoning as app_update_battery_percent()'s own fix: NRF_READY
     * only means "poll the nRF for new chip reads" (see
     * process_nrf_spi()'s own use of it), not a general bus-safety/
     * scheduling-priority signal for unrelated GPS SPI work.
     * checkInterval2 now always advances on the pass this runs. */
#if APP_ENABLE_GPS
