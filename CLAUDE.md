...
- `real_project_config/` -- **mirrored, NOT live-synced** copies of the
  real-project-only config files that most frequently hold the actual
  bug (`.cproject`, `lwipopts.h`, `board/pin_mux.c`,
  `board/peripherals.c`, `board/clock_config.c`) -- added 2026-07-21
  specifically because these files were previously untracked ANYWHERE,
  meaning there was no "before" state to diff against when something
  broke. `board/clock_config.c` added 2026-07-22 after it turned out to
  hold the actual root cause of the entire ENET/PHY bring-up saga (see
  project memory: `clock_config.c` was generated by a mismatched/newer
  MCUXpresso Config Tools version that unconditionally forced a 600MHz
  clock path instead of checking the chip's own OCOTP speed-grade fuse).
  Same manual-copy workflow as `firmware_source/`'s own real-project
  sync: after editing any of these files in the real MCUXpresso project
